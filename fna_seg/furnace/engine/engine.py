...
class Engine(object):
    ...
    def inject_default_parser(self):
        ...
        p.add_argument('-arch', '--arch_name', default='171048_48',
                       help='which architect')
    ...
    def save_and_link_checkpoint(self, snapshot_dir, log_dir, log_dir_link):
        ...
        current_idx = self.state.epoch * 1000 + self.state.iteration + 1
    ...
    def save_and_link_checkpoint_epoch(self, snapshot_dir, log_dir, log_dir_link):
        ensure_dir(snapshot_dir)
        if not osp.exists(log_dir_link):
            link_file(log_dir, log_dir_link)
        current_epoch_checkpoint = osp.join(snapshot_dir, 'epoch-{}.pth'.format(
