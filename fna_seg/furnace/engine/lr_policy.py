...
class PolyLR(BaseLR):
    def __init__(self, start_lr, lr_power, total_iters, warmup_iters):
        self.start_lr = start_lr
        self.lr_power = lr_power
        self.warmup_iters = warmup_iters
        self.total_iters = total_iters + 0.0 - warmup_iters

    def get_lr(self, cur_iter):
        if cur_iter < self.warmup_iters:
            lr = 0.0001 + (self.start_lr - 0.0001) / (self.warmup_iters) * (cur_iter)
        else :
            cur_iter = cur_iter - self.warmup_iters
